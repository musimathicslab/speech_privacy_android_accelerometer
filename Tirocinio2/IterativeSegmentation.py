...
def openDir(directory, file, temps):
    t = []
    x = []
    y = []
    z = []
...
if __name__ == '__main__':
    ...
    while temp < 0.2:
        print(f'Test: {temp}[')
        speeches, accuracy, efficacy = openDir('SegmentationFile', 'frase1.csv', temp)
        # INVIO RISULTATI
        ...
        print(f']')
        result.append(speeches)
        result_acc.append(accuracy)
        result_efficacy.append(efficacy)
        temp = temp + 0.1
    ...
    M = 0
    ind = 0
    for i in range(0, len(result_acc)):
        if result_acc[i] > M:
            M = result_acc[i]
            ind = i

    # INVIO RISULTATI
    print(f'Best test:{ind + 1}, -accuracy: {result_acc[ind]}%, -efficacy: {result_efficacy[ind]}%')
    print(f'SPEECH Found :')
    for element in result[ind]:
        print(f'{element}')
