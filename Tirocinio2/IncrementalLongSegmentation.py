...
def segmentationDir(t, x, y, z, i):
    ...
    if len(x1) >= 50:
        ...
        acc_prediction.append(acc_y_pred)
        eff_prediction.append(efficacy_pred)
        tmp_prediction.append(temp)
        ind_prediction.append(i)
    ...
    for j in range(1, len(prediction)):
        ...
        if eff_prediction[j] == efficacy_pred:
            if acc_prediction[j] >= acc_y_pred:
                y_pred = prediction[j]
                acc_y_pred = acc_prediction[j]
                efficacy_pred = eff_prediction[j]
                temp = tmp_prediction[j]
                i = ind_prediction[j]
...
def openDir(directory, file):
    ...
    while i < len(x):
        print(f'Iteration: {u}[')
        u = u+1
        i1, y_pred, acc_y_pred, efficacy_pred, t1 = segmentationDir(t, x, y, z, i)
        i = i1
